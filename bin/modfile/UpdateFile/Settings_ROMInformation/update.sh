...
mods "Add ROM Information To HalcyonOS"
  mkdir -p $WORK_DIR/apk_temp
  isSettingsDIR=$(find "$MAIN_FOLDER" -type d -name "Settings")
  isSettings=$(find "$MAIN_FOLDER" -type f -name "Settings.apk")
  $APKEDITOR d -i $isSettings -o $WORK_DIR/apk_temp/isSettings.apk.out >/dev/null 2>&1
  ...
  echo "ro.HalcyonOS.version=HalcyonOS $myversion | $final_version" >> $my
  echo "ro.HalcyonOS.osversion=${simposcode}.${build_date}" >> $my
  echo "ro.HalcyonOS.simposcode=HalcyonOS 1.2 $myversion  " >> $my
  ...
  rm -rf $WORK_DIR/apk_temp
  mods "Adding HalcyonOS Information Done!"
